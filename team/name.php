...
$name = isset($_POST['name']) ? trim($_POST['name']) : '';
    ...
    if (mb_strlen($name) < 5){
    echo '<div class="rmenu"><p>Слишком короткое название!</p></div>';
    echo '<a href="index.php">В менеджер</a>';
    require_once ("../incfiles/end.php");
    exit;
}
...
mysql_query("UPDATE `team_2` SET `name` = '$name', `butcer`=`butcer`-'$num' WHERE `id` = '$id';");

//mysql_query("UPDATE `team_2` SET `name` = '$name' WHERE `id` = '$id';");                    
                    
echo '<div class="gmenu"><p style="text-align:center;">Имя команды изменено<br/>
