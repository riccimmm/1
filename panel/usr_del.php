...
if (!$error) {
    ...
    switch ($mod) {
        case 'del' :
			...
			mysql_query("UPDATE `mail_msg` SET `read`='2', `delete` = '" . $user['id'] . "' WHERE `id_user` = '" . $user['id'] . "' OR `id_kont` = '" . $user['id'] . "'");
			mysql_query("DELETE FROM `mail_konts` WHERE `id_user` = '" . $user['id'] . "' OR `id_kont` = '" . $user['id'] . "'");
			mysql_query("DELETE FROM `mail_msg` WHERE `id_kont` = '" . $user['id'] . "' AND `sys`='2'");
			mysql_query("OPTIMIZE TABLE `mail_konts`");
            ...
            echo '<div class="rmenu"><h3>Пользователь удален!</h3></div>';
            break;
...
echo '<div class="menu"><a href="index.php?act=usr_list">Список пользователей</a></div>
<div class="menu"><a href="index.php">Админ панель</a></div>';
