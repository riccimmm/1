...
function usersonline() {
    ...
    return ($user_id ? '<a href="' . $home . '/str/online.php">Online: ' . $users . '</a>' : 'Online: ' . $users . '');
}
...
function antilink($var) {
    ...
    ////////////////////////////////////////////////////////////
    //TODO: Убрать eregi
    //$var = replace("((https?|ftp)://)([[:alnum:]_=/-]+(\\.[[:alnum:]_=/-]+)*(/[[:alnum:]+&._=/~%]*(\\?[[:alnum:]?+&_=/;%]*)?)?)", "[реклама]", $var);
    $var = strtr($var, array (
        ".ru" => "***",
        ".com" => "***",
        ".net" => "***",
...
$out .= '<img style="border-radius:5px;" src="../files/photo/' . $user['id'] . '.jpg" alt="' . $user['name'] . '" width="64px" height="64px"/>&nbsp;';

}else{

$out .= ' <img style="border-radius:5px;" src="../files/photo/nophoto.jpg" alt="' . $user['name'] . '" width="64px" height="64px"/>&nbsp;';
        ...
        $out .= ($realtime > $user['lastdate'] + 300 ? '<span class="red"> [Off]</span>' : '<img src="'.$home.'/images/icons/on.png" />');
        if (!empty($str))
            $out .= ' ' . $str;
