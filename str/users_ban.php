...
switch ($act) {
    ...
    case 'details':
        ...
        if (mysql_num_rows($req)) {
            ...
            echo '<div class="phdr">Всего нарушений: ' . $total . '</div>';
            echo '<p>';
            if ($rights >= 7 && $res['ban_time'] > $realtime) echo '<a href="users_ban.php?act=razban&amp;id=' . $user['id'] . '&amp;ban=' . $ban . '">Разбанить</a><br />';
            if ($rights == 9) echo '<a href="users_ban.php?act=delban&amp;id=' . $user['id'] . '&amp;ban=' . $ban . '">Удалить бан</a><br />';
        ...
        if (!$ban || $rights < 7) echo 'ошибка доступа';
            ...
            if (mysql_num_rows($req)) {
                ...
                if (isset($_POST['submit'])) {
                    ...
                    <a href="users_ban.php?act=violation&amp;id=' . $id . '">Вернуться</a></p></div>';
                } else {
                    echo '<form action="users_ban.php?act=delViolation&amp;id=' . $id . '&amp;ban=' . $ban . '" method="POST">' . '<div class="menu"><p>Удалить предупреждение?</p>' . '<p><input type="submit" name="submit" value="Удалить" /></p>' . '</div></form>' . '<div class="phdr"><a href="users_ban.php?act=violation&amp;id=' . $id . '">Назад</a></div>';
                }
            } else {
        ...
        */
        ...
        if ($total) {
            ...
            while ($res = mysql_fetch_assoc($req)) {
                ...
                if ($rights >= 7) echo '<a href="users_ban.php?act=delViolation&amp;id=' . $user['id'] . '&amp;ban=' . $res['id'] . '">Удалить предупреждение</a>';
                echo '</div></div>';
                ++$i;
            }
        } else {
        ...
        if ($total > $kmess) {
            echo '<div class="bmenu">' . pagenav('users_ban.php?act=violation&amp;id=' . $id . '&amp;', $start, $total, $kmess) . '</div>' . '<p><form action="users_ban.php?act=violation&amp;id=' . $id . '&amp;" method="post">' . '<input type="text" name="page" size="2"/>' . '<input type="submit" value="' . $lng['to_page'] . ' &gt;&gt;"/></form></p>';
        }
        break;
    default:
        ////////////////////////////////////////////////////////////
