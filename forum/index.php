        ...
        switch ($tip) {
            case 'f':
                ...
                while ($res = mysql_fetch_assoc($req)) {
                    ...
                    if ($coltem)
                        echo " [$coltem]";
                    if (!empty($res['soft']))
                        echo '<div class="d"><span class="gray">' . $res['soft'] . '</span></div>';
                    echo '</div>';
                    ++$i;
                }
            ...
            case 'r':
                ...
                if ($user_id && !$ban['1'] && !$ban['11']) {
                    echo '<div class="b"><form action="index.php?act=nt&amp;id=' . $id . '" method="post"><center><input type="submit" value="Создать новую тему" /></center></form></div>';
                }
                ...
                while ($mass = mysql_fetch_assoc($q1)) {
                    ...
                    echo '<div class="d"><small>';
                    echo $mass['from'];
                    if (!empty($nam['from'])) {
                        echo '&nbsp;/&nbsp;' . $nam['from'];
                    }
                    $vrp = $mass['time'] + $set_user['sdvig'] * 3600;
                    echo ' <font color="#777777">' . date("d.m.y / H:i", $vrp) . "</font></small></div></div>";
                    ++$i;
                }
            ...
            case 't':
                ...
                while ($res = mysql_fetch_assoc($req)) {
                    ...
                    if ($set_user['avatar']) {
						...
						if (file_exists(('../files/photo/' . $res['user_id'] . '.jpg'))){
echo '<img style="border-radius:5px;" src="../files/photo/' . $res['user_id'] . '.jpg" alt="' . $res['name'] . '" width="45px" height="45px"/>&nbsp;';
}else{
echo ' <img style="border-radius:5px;" src="../files/photo/nophoto.jpg" alt="' . $res['name'] . '" width="45px" height="45px"/>&nbsp;';
}
        ...
        while ($res = mysql_fetch_array($req)) {
            echo ($i % 2) ? '<div class="at">' : '<div class="at">';
    ...
    if ($id && $tip == 'f') {
        ...
        if ($count > 0)
            ...
            echo '<div class="c">Прикрепленных файлов нет</div>';
    } elseif ($id && $tip == 'r') {
